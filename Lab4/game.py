...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    return rooms[exits.get(direction)]["name"]
...
def print_menu_line(direction, leads_to):
    ...
    """
    print( "Go {0} to {1}.".format(direction.upper(),leads_to))
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    if (( user_input in ['up','north']) and 'north' in exits):
     return True
    elif (( user_input in ['right','east']) and 'east' in exits):
     return True
    elif (( user_input in ['down','south']) and 'south' in exits):
     return True
    elif (( user_input in ['left','west']) and 'west' in exits):
     return True
    else:
     return False
...
def menu(exits):
    ...
    while True:
        ...
        # Normalise the input
        choice = normalise_input(raw_choice)
		
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if is_valid_exit(exits,choice):
            system("cls")
            print("You go to " + exit_leads_to(exits,choice))
            return choice
